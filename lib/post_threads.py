...
Threads API notes (confirmed against Meta's official changelog, checked July 2026):
    ...
    - Replies are supported via `reply_to_id` on container creation — see
      reply_to_post() below. daily_post.py uses this to post the outbound
      site link as the FIRST REPLY rather than in the main post body, since
      Threads is widely reported to suppress reach on posts whose main body
      contains an outbound link.
"""
...
def reply_to_post(parent_id: str, text: str, timeout: int = 30) -> dict:
    ...
    try:
        ...
        raise ThreadsError(f"Failed to create Threads reply container: {e} / {create_resp.text}") from e
    ...
    try:
        ...
        raise ThreadsError(f"Failed to publish Threads reply: {e} / {publish_resp.text}") from e
