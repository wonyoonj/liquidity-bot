# -*- coding: utf-8 -*-
"""
웹사이트(index.html)의 updateLiquidityFlowAnimation() 안에 있는
"시장 Total 유동 공급량(Net Market Flow)" 계산 공식을 파이썬으로 그대로 재구현.

netMarketFlow = fedLiquidityDiff - tgaDiff + finalMmfToMarketDiff

세부 공식은 웹사이트 코드 주석을 참고해 1:1로 옮겼습니다.
"""
...
def _closest(data: Series, target_date: datetime) -> Optional[float]:
    """target_date 에 가장 가까운 값을 반환. (JS getClosestData()와 동일한 동작)"""
    if not data:
        return None
...
def compute_net_market_flow(data_store: dict, as_of: Optional[datetime] = None) -> dict:
    """
    data_store: fetch_data.fetch_all() 이 반환한 dict
    as_of: 기준 날짜 (기본값 = 데이터의 가장 최근 날짜). 추세 비교용으로 과거 날짜를 넣을 수도 있음.
    """
    required = ["WTREGEN", "WALCL", "RRPONTSYD", "WRESBAL",
                "MMF2MARKET", "MMF2GOVERNMENT", "MMMFFAQ027S"]
    for key in required:
        if not data_store.get(key):
            raise ValueError(f"'{key}' 데이터가 비어 있습니다.")
    ...
    if None in (tga_now, tga_past, walcl_now, walcl_past, rrp_now, rrp_past, resbal_now, resbal_past):
        raise ValueError("TGA/WALCL/RRP/WRESBAL 계산에 필요한 데이터가 부족합니다.")
    ...
    if None in (latest_m, past_m, latest_g, past_g):
        raise ValueError("MMF2MARKET/MMF2GOVERNMENT 계산에 필요한 데이터가 부족합니다.")
    ...
    if None in (latest_t, past_t):
        raise ValueError("MMMFFAQ027S 계산에 필요한 데이터가 부족합니다.")
...
def classify_state(net_market_flow: float) -> dict:
    """웹사이트의 today-summary-banner 색상/문구 로직과 동일한 5단계 분류."""
...
def compute_trend_text(data_store: dict, current_result: dict) -> str:
    """4주(약 28일) 전과 비교한 추세 문구. 실패해도 전체 실행은 막지 않음."""
    try:
        ...
        if abs(diff) <= 5:
            return ""
        return " · 한 달 전보다 공급 속도 ↑" if diff > 0 else " · 한 달 전보다 공급 속도 ↓"
    except Exception:
        return ""
